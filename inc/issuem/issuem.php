 ...
 */
function ufclas_emily_issuem_default_settings( $defaults ){
...
add_filter( 'issuem_default_settings', 'ufclas_emily_issuem_default_settings' );

/**
 * Add custom templates
 * @return string
 */
function ufclas_emily_issuem_templates( $template_path ){
	...
	if ( is_singular('article') ){
		...
		// Change template for the newsletter page
		$template_path = get_stylesheet_directory() . '/inc/issuem/issuem-page.php';
...
add_filter( 'template_include', 'ufclas_emily_issuem_templates', 1 );
 ...
 * @return string
 */
function ufclas_emily_issuem_newsletter_title(){
	$issuem_settings = get_issuem_settings();
	$current_issue = ufclas_emily_issuem_issue_data();
...
function ufclas_emily_issuem_get_categories(){
	$terms = array();
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_setup(){
	register_nav_menus( array(
		'newsletter-menu' => esc_html__( 'Newsletter Menu', 'ufclas-ufl-emily' ),
	) );

}
add_action( 'after_setup_theme', 'ufclas_emily_issuem_setup' );
 ...
 */
function ufclas_emily_issuem_query( $query ) {
...
add_action( 'pre_get_posts', 'ufclas_emily_issuem_query', 1 );
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_article_posts_columns( $columns ){
	unset($columns['comments']);
	unset($columns['expirationdate']);
	return $columns;
}
add_filter('manage_article_posts_columns', 'ufclas_emily_issuem_article_posts_columns');
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_article_filter_list() {
...
add_action( 'restrict_manage_posts', 'ufclas_emily_issuem_article_filter_list' );
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_article_filter( $query ) {
	if ( is_admin() ){
		$qv = &$query->query_vars;
...
add_filter( 'parse_query','ufclas_emily_issuem_article_filter' );
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_readmore( $more ) {
	return '<p class="read-more"><a class="read-more" href="'. get_permalink( get_the_ID() ) . '">' . __('Read More', 'ufclas-ufl-emily') . '</a></p>';
}
add_filter('excerpt_more', 'ufclas_emily_issuem_readmore');
add_filter( 'the_content_more_link', 'ufclas_emily_issuem_readmore' );
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_issue_data() {
	global $post;
 ...
 * @since 1.1.0
 */
function ufclas_emily_issuem_posted_on() {
 ...
 * @since 1.2.0
 */
function ufclas_emily_issuem_title( $title, $sep ) {
	$issuem_settings = get_issuem_settings();
...
add_filter( 'wp_title', 'ufclas_emily_issuem_title', 10, 2 );
 ...
 * @since 0.7.0
 */
function ufclas_emily_newsletter_data() {
	global $post;
 ...
 * @since 0.7.0
 */
function ufclas_emily_newsletter_classes( $classes ) {
...
add_filter( 'body_class', 'ufclas_emily_newsletter_classes' );
...
*
*
* CSS Styling Insertion
*
*
*=============================*/
function issuemCSS(){
  wp_enqueue_style( 'issuem-css', get_theme_file_uri() . '/inc/issuem/css/issuem-css.css' );
}

add_action( 'wp_enqueue_scripts', 'issuemCSS' );
